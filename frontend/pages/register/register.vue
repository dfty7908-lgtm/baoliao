<template>
    <view class="container">
        ...
        <view class="form-card">
            ...
            <up-button :loading="loading" text="立即注册"
                ...
                @click="handleRegister"></up-button>
...
onLoad((options: any) => {
    if (options.code) {
        form.inviteCode = options.code;
    } else {
        // Fallback to stored code from landing
        const storedCode = uni.getStorageSync('inviteCode');
        if (storedCode) {
            form.inviteCode = storedCode;
        }
    }
});
...
const getCode = () => {
    ...
    // 调用发送验证码接口
    sendSms(form.phone, 'REGISTER').then((res: any) => {
        if (res.code === 200) {
